...
from subhunter.core.active.dns_resolve import bulk_resolve
from subhunter.core.active.http_probe import HTTPProbeResult, bulk_probe
from subhunter.core.passive.chaos import fetch_chaos
from subhunter.core.passive.crtsh import fetch_crtsh
from subhunter.core.passive.securitytrails import fetch_securitytrails
from subhunter.core.passive.virustotal import fetch_virustotal
from subhunter.core.passive.wayback import fetch_wayback
from subhunter.core.utils.config import APIConfig, RuntimeConfig, load_api_config
from subhunter.core.utils.db import SubdomainRecord, get_historical_subdomains, init_db, upsert_subdomain
from subhunter.core.utils.dedupe import dedupe_subdomains
from subhunter.core.utils.output import emit_json, emit_subdomain_list, emit_txt, log_error, log_info
...
async def classify_and_persist(
    ...
    if passive_subs:
        log_info(f"[*] Performing DNS resolution on {len(passive_subs)} hosts...", silent=rt_cfg.silent)
        dns_results = await bulk_resolve(sorted(passive_subs), concurrency=rt_cfg.concurrency)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="subhunter",
        description=(
            "SubHunter - asynchronous subdomain reconnaissance with passive OSINT, "
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    log_info(f"[*] Starting SubHunter against {rt_cfg.domain}", silent=rt_cfg.silent)
